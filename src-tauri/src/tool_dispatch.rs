...
use crate::{
    ...
    runtime_tool, shell_tool, string_argument, workspace_root_for_arguments, AgentSettings,
    AppState,
};
...
pub(crate) async fn run(
    ...
    match name {
        ...
        "rust_shell" => shell_tool::run(arguments, Some(&workspace)).await,
        "rust_bash" => {
            shell_tool::run_persistent(state, task_id, arguments, None, &workspace).await
        }
        "rust_sandbox_shell" => {
            shell_tool::run_persistent(state, task_id, arguments, Some("sandbox"), &workspace).await
        }
        "rust_code" => {
        ...
        "rust_files" => file_tool::run(arguments, external_path_approved, &workspace).await,
        "rust_sandbox_files" => {
            file_tool::run_sandbox(task_id, arguments, external_path_approved, &workspace).await
        }
        "rust_str_replace_editor" => {
            editor_tool::run(state, arguments, external_path_approved, &workspace).await
        }
        ...
        "rust_browser_use" => {
            crate::run_browser_tool(state, arguments, "browser", &workspace).await
        }
        "rust_sandbox_browser" => {
            crate::run_browser_tool(state, arguments, "sandbox_browser", &workspace).await
        }
        "rust_computer_use" => {
            computer_tool::run(arguments, external_path_approved, &workspace).await
        }
        "rust_python_execute" => runtime_tool::run_python(arguments, &workspace).await,
        ...
        "rust_visualization_preparation" => {
            data_tool::run_visualization_preparation(arguments, external_path_approved, &workspace)
        }
        "rust_data_analysis" => data_tool::run_data_analysis_tool(arguments, &workspace).await,
        "rust_data_visualization" => {
            data_tool::run_data_visualization_tool(arguments, &workspace).await
        }
        "rust_sandbox_vision" => {
            runtime_tool::run_sandbox_vision(task_id, arguments, &workspace).await
        }
        "rust_terminate" => {
            let status =
